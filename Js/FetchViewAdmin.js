...
fetch("http://localhost:5000/api/Service/FetchViewServices")
    .then((response) => response.json())
    .then((data) => {
        const tableBody = document.getElementById("ViewData");

        // Iterate through the data array and create table rows dynamically
        data.map((ViewCount, index) => {
            const newRow = document.createElement("tr");
            newRow.innerHTML = `
        <td>${index + 1}</td>
        <td>${ViewCount.Title}</td>
        <td>${ViewCount.Quantity}</td>
        <td>${ViewCount.Links}</td>
        <td>
          <center><input type="checkbox" id="subscribercheck" data-id="${ViewCount._id}" /></center>
        </td>
      `;
...
async function ClearAll() {
    const response = await fetch(`http://localhost:5000/api/Service/DeleteViewService`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json"
            },
